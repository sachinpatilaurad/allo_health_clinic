...
export default function LoginPage() {
  // --- THIS IS THE FIX ---
  // The initial state for email and password is now an empty string.
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.data && response.data.access_token) {
        localStorage.setItem('accessToken', response.data.access_token);
        // I see you've already updated the redirect to go to /doctors, which is perfect.
        router.push('/doctors'); 
      }
    } catch (err) {
      console.error('Login failed:', err);
      ...
      <div className="w-full max-w-md p-8 space-y-8 bg-white rounded-lg shadow-md">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <button
              type="submit"
              // The button is disabled while loading OR if the fields are empty
